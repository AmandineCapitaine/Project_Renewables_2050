...
class opti_model : 
    ...
    def load_param(self, path, name_load_factor, name_load_demand) : 
    ...
    def write_results(self, model_name, name_load_demand, name_load_factor):
        ...
        C_index += [data_C.index]
        C_value += [pyo.value(data_C)]
        
        for i in self.tec:
            C_index += [i]
            C_value += [pyo.value(self.cost_by_tec(i))]
        
        C_file = model_name + "/" + model_name + "_Cost.csv"
        data_C = pd.DataFrame(C_value, index=C_index)
